...
pacman::p_load(dplyr, ggplot2, glmnet, car, data.table, skimr)   #add your packages here
...
* A quick summary about the data.
  This dataset comes from the Center for Clinical and Transnational Research (https://archive.ics.uci.edu/ml/datasets/Diabetes+130-US+hospitals+for+years+1999-2008) at Virginia Commonwealth that covers data on diabetes patients across 130 U.S. hospitals from 1999 to 2008.
...
dia.x <- model.matrix(readmitted~., data=dia.cleaned)[, -1]
dia.y <- as.matrix(dia.cleaned$readmitted)

#fit.cv <- cv.glmnet(x = dia.x, y = dia.y, alpha=1, family="binomial", nfolds = 10, type.measure = "deviance")  
...
```{r}
fit.logit <- glm(readmitted ~ time_in_hospital+num_medications+number_emergency+number_inpatient+number_diagnoses+diabetesMed+disch_disp_modified+age_mod+diag1_mod+diag3_mod, data = dia.cleaned, family=binomial)
Anova(fit.logit)
```

```{r}
fit.logit.new <- glm(readmitted ~ +number_emergency+number_inpatient+number_diagnoses+diabetesMed+disch_disp_modified+age_mod+diag1_mod+diag3_mod, data = dia.cleaned, family=binomial)
Anova(fit.logit.new)
anova(fit.logit.new, fit.logit, test="Chisq")
```

```{r}
summary(fit.logit.new)
```
