...
ggplot(radio) + 
  geom_histogram(aes(x = age), fill = "blue") +
...
ggplot(radio) +
  geom_bar(aes(x = income), fill = "blue")
radio %>%
  group_by(income) %>% 
  summarise(cnt = n()) %>% 
  mutate(freq = cnt / sum(cnt)) %>% 
...
ggplot(radio) +
  geom_bar(aes(x = sirius), fill = "blue")
radio %>%
  group_by(sirius) %>% 
  summarise(cnt = n()) %>% 
  mutate(freq = cnt / sum(cnt)) %>% 
...
ggplot(radio) +
  geom_bar(aes(x = wharton), fill = "blue")
radio %>%
  group_by(wharton) %>% 
  summarise(cnt = n()) %>% 
  mutate(freq = cnt / sum(cnt)) %>% 
...
# Approximately 57% of people on MTurk identify as female, according to the Census Bureau, women represent just under 51% of the U.S. population.
radio %>%
  group_by(gender) %>% 
  summarise(cnt = n()) %>% 
  mutate(freq = cnt / sum(cnt)) %>% 
...
#our sample is 57.5% male and 42.5% female. Given the national sample and MTurk sample, our sample has sligtly more male than female.

#About 30% of people on MTurm are in their 20s, 37% of people are in their 30’s, another 17% are in their 40’s, and roughly 11% are in their 50’s

radio <- radio %>% 
  mutate(agegroup = case_when(age >= 18 & age <= 29 ~ '1',
                              age >= 30 & age <= 39 ~ '2',
                              age >= 40 & age <= 49 ~ '3',
                              age >= 50 & age <= 59 ~ '4',
                              age >= 60 ~ '5'))
...
radio %>%
  ...
  mutate(freq = cnt / sum(cnt)) %>% 
  arrange(desc(freq))
#In our sample, 59% are in their 20s, 25% are in their 30s, 9% are in their 40s, 5% are in their 50s, and only 1% in their 60s or over. Our sample is slighly more young skewed than the general population

#MTurk: 46.42% less than 50k, 26.46% 50k-80k, 22.2% 80k-150k, 4.92% > 150k
#National: 40% less than 50k, 20.2% 50k-80k, 24.35% 80k - 160k, 15.47% > 150k
radio %>%
  group_by(income) %>% 
  summarise(cnt = n()) %>% 
  mutate(freq = cnt / sum(cnt)) %>% 
...
#Our: 57.3% < 50k, 22% 50k to 75k, 19% 75k to 150k, and 3% > 150k
...
In the existing survey data, we have 4 independent demographic varibles `income`, `gender`, `education` and `age`. To study the significance of each variable on the probability of listening Wharton Talk Show ($P(X)$), we performed a multinomial logistic regression as follow 
...
However ...... We have to simplify the model into the following
$$
P(X) = P(X|M)P(M) + P(X|\bar{M})P(\bar{M})
$$
```{r, echo = FALSE, warning = FALSE}
# the goal is to estimate the proportion of the Wharton listeners to that of the Sirius listener, so that we will come up with an audience size estimate of approximately 51.6 million times
#calculate four categories with possibilities
P_XM <- radio %>% 
...
In 2015, there is more female compared to male within `Non S&E` fields. However, within the science-related fields, the number of female is almost identical compared to male.


%% Are we doing analysis on gender effects over B.S, M.S. and Ph.D or degrees in the field
...
Furthermore, from the graph, we can see gender effect on degree for both science & engineering related fields and non science & engineering related fields, especially the latter one, as stated in the previous paragraph.
...
Based on the analysis, the female is underrepresented in science and engineering related fields over the past 10 years. While in this study, the number of degree awarded for female may be similar to the male at a given year, the ratio/percentage is not. 

In this data set, for each year, more degrees are awarded to the female compare to the male. In other words, the data set has more female samples compare to male samples. While this could be true, one improvement is that: instead of arbitrarily selecting samples, the ratio between the number of degrees awarded to female and male for a given year should reflect the actual ratio.
