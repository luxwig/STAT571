...
The above distribution for team payroll shows that the max values are significant outliers. A logarithmic transformation is more appropriate in this set up to handle skewness in the data.
...
```{r, include=TRUE,echo=TRUE}
baseball_lag %>%
  ggplot(aes(x = diff_lag, y = win_pct, color = team)) +
  ...
  geom_smooth(
    ...
    color = "red") +
  theme_bw() +
  labs(x = "Yearly Increase in Payroll",
...
Using root mean square error as an evaluation metric, it looks like yearly increase in payroll is a better fit for the data than yearly payroll. Using R-squared an evaluation metric, however, shows that yearly payroll explains more variance in the data than yearly increase in payroll.
