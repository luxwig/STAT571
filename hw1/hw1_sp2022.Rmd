...
Similar to the analysis methods from the previous section, we will first identify the variables of interests. We will then estimated the probability $P(X)$ through the same methods as indicated in the ``Estimation Methods``.

Since the the demographics of the participants is sampled from across U.S., the demographics of the participants will be much closer to the national demographics, which is available through the U.S. Census Bureau. By slicing the data from different perspective, the prior probability (such as $P(M \cap G)$ and $P(M \cap \bar{G})$ from the previous example) can be estimated more accurately.
