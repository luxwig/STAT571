...
pacman::p_load(ISLR, data.table,skimr,ggcorrplot,knitr,factoextra,conflicted,fastDummies,caret,stargazer,irlba) # add the packages needed
...
```{r eval=F}
#use prcomp result for this part of the pca result 
...
#predict(brca_sub_pca, x_patient)
```
...
The second model (ii) has a narrower 95% CI compared to the first one (i). The second model uses two variables to predict the outcome (MPG) instead of on one, which improve the fitness overall. With a better fitness, we are more confident about our estimation of the relationship between the year and MPG, which results in a narrower CI.
...
fit5 <- aov(mpg ~ cylinders, data = autoLvData)
summary(fit5)
```
...
```{r}
finalFit <- lm(log(mpg) ~  horsepower + weight + year + origin, data = autoData)
