...
```{r eval=F}
system.time({brca_sub_pca <- prcomp(brca_sub, scale. = F)}) 
saveRDS(brca_sub_pca, "brca_sub_pca.RDS")
```
...
x_patient <- x_patient[, sel_cols_x, with=F]

# Get rid of columns without variability
which(apply(x_patient, 2, var) == 0)

# Apply logarithmic transformation
x_patient <- log2(as.matrix(x_patient+1e-10))

x_patient_co <- as.data.frame(x_patient[ , which(colnames(x_patient) %in% colnames(brca_sub))])
                                  
x_scaled <- scale(x_patient_common, brca_sub_pca$center, brca_sub_pca$scale) %*% brca_sub_c.pca$rotation
predict(brca_sub_pca, x_patient_co)
```
