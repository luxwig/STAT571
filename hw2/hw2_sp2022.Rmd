...
Anova(lm(mpg ~ cylinders, data = autoLvData))
summary(lm(mpg ~ cylinders, data = autoLvData))
```

The `cylinders` is significant at the 0.01 level. The MPG are significant difference between at least two cylinder groups.
...
While treating `cylinders` as a continuous variable, we are testing whether MPG has a linear relationship with `cylinders`. However, when treating `cylinders` as a categorical variable, we are testing whether MPG is different between at least two different cylinder groups.
...
```{r}
finalFit <- lm(log(mpg) ~  horsepower + weight + year + as.factor(origin) + year*horsepower + as.factor(cylinders), data = autoData)

finalRes = resid(finalFit)
summary(finalFit)
